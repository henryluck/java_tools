package com.jrebel.plugin.dam.cbp;

import org.zeroturnaround.bundled.javassist.ClassPool;
import org.zeroturnaround.bundled.javassist.CtClass;
import org.zeroturnaround.bundled.javassist.CtMethod;
import org.zeroturnaround.javarebel.integration.support.JavassistClassBytecodeProcessor;

import com.jrebel.plugin.dam.ClassUtils;

public class DAMFactoryCBP extends JavassistClassBytecodeProcessor {
    //奇怪，cbp类会被调用3次，控制一下

    public DAMFactoryCBP() {
        // TODO Auto-generated constructor stub
    }

    @Override
    public void process(final ClassPool cp, final ClassLoader arg1, final CtClass ctClass) throws Exception {
        //奇怪，cbp类总是被调用多次，导致错误,验证一下
        if(ClassUtils.isExecuted(ctClass)){
            return;
        }
        cp.importPackage("com.jrebel.plugin.dam");
        //加载所有的xml
        CtMethod initialize = ctClass.getDeclaredMethod("initialize");
        initialize.insertAfter("Debuger.log(\"JRebel dam plugin: DAMFactory process,Start register xml files ......\");"+
                                            "com.jrebel.plugin.dam.SqlMapXmlFilesManager.registerXml(getClass().getClassLoader());"+
                                            "com.jrebel.plugin.dam.SqlMapXmlFilesManager.dam=this;"+
                                            "Debuger.log(\"JRebel dam plugin: DAMFactory process,register xml files finish!!\");");
    }

}
